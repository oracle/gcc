	  ...
	  if (CTF_V2_INFO_KIND (ref_type->dtd_data.ctti_info) == CTF_K_SLICE)
	    {
	      ...
	      /*  For bitfield members, the offset and bitfield size are packed
		  together into a 32-bit word. The offset cannot be more than
		  24 bits...  */
	      if (sou_offset + word_offset > 0xffffff)
		warning (OPT_Woverflow, "BTF bitfield offset does not fit in 24 bits");

	      /* ... and the bit size cannot be more than 8 bits (255).  */
	      if (bits > 0xff)
		warning (OPT_Woverflow, "BTF bitfield more than 255 bits");
