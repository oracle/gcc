...
#include "cgraph.h"
#include "varasm.h"
#include "dwarf2int.h" /* For lookup_decl_die.  */
...
hash_map <ctf_dvdef_ref, unsigned int> *btf_var_ids;
...
btf_calc_num_vbytes (ctf_dtdef_ref dtd)
{
  ...
  uint32_t kind = BTF_INFO_KIND (dtd->dtd_data.ctti_info);
  uint32_t vlen = BTF_INFO_VLEN (dtd->dtd_data.ctti_info);
			...
			struct btf_var_secinfo info)
{
  ...
  uint32_t str_off;
  ctf_add_string (ctfc, secname, &str_off, CTF_STRTAB /*aux_string?*/);
  if (strcmp (secname, ""))
    ctfc->ctfc_strlen += strlen (secname) + 1;
  ...
  FOR_EACH_VARIABLE (node)
      ...
      if (section_name == NULL)
	{
	  switch (categorize_decl_for_section (node->decl, 0))
	    ...
	    default:
	      continue;
	    /* FIXME - are these really the only sections supported?  */
	    }
	}
	...
	info.type = *var_id + ctfc->ctfc_num_types + 1;
      else
	continue;
...
btf_adjust_type_ref (ctf_dtdef_ref dtd)
{
  ...
  dtd->dtd_type = btf_adjust_type_id (dtd->dtd_type);
  ...
  switch (btf_kind)
	...
	for (farg = dtd->dtd_u.dtu_argv;
	  ...
	  {
	    /* A function argument whose type is not representable is encoded
	       as having type_id=0, i.e., void.  */
	    if (btf_removed_type_p (farg->farg_type))
	      farg->farg_type = BTF_VOID_TYPEID;
	    else
...
/* Adjust the type ID referenced by a variable to account for removed types.  */
...
/* Process struct or union member DMD to ensure its validity and re-encode it
   into BTF format if needed. This catches members referring to unsupported
   types and bit-field members which must be re-encoded or which cannot be
   represented in BTF.  */

static void
btf_preprocess_sou_member (ctf_container_ref ctfc, ctf_dmdef_t *dmd)
{
  ctf_dtdef_ref ref_type;

  /* Use the dmd_value field, which is otherwise only used for enum values,
     to keep track of bitfield members.  */
  dmd->dmd_value = 0;
  ref_type = ctfc->ctfc_types_list [dmd->dmd_type];

  /* If the member type is not understood by the CTF container, do not attempt
     to read any information from it; skip it.  */
  if (ref_type == NULL)
    dmd->dmd_type = BTF_INVALID_TYPEID;

  /* Bitfields.  */
  else if (CTF_V2_INFO_KIND (ref_type->dtd_data.ctti_info) == CTF_K_SLICE)
    {
      ...
      /*  For bitfield members, the offset and bitfield size are packed together
	  into a 32-bit word. The offset cannot be more than 24 bits, and the
	  bit size cannot be more than 8 bits. If either of these conditions are
	  violated, the bitfield cannot be represented in BTF, and should be
	  skipped.  */
      if (bits > 0xff || sou_offset + word_offset > 0xffffff)
	dmd->dmd_type = BTF_INVALID_TYPEID;
    }

  /* Do not output members which refer to deleted types.
     Note: this must be after bitfield handling, because CTF_K_SLICE types
     (bitfields) are removed types.  */
  else if (btf_removed_type_p (dmd->dmd_type))
    dmd->dmd_type = BTF_INVALID_TYPEID;
}


/* Preprocessing for struct and union types. This must be performed after the
   initial types list is constructed, because struct/union members only refer
   to type by ID.  */

static void
btf_preprocess_sous (ctf_container_ref ctfc)
{
  size_t i;
  unsigned int ctf_kind, btf_kind;
  ctf_dtdef_ref type;

  /* Type ID 0 is sentinel, not real type.  */
  for (i = 1; i <= ctfc->ctfc_types->elements (); i++)
    {
      type = ctfc->ctfc_types_list [i];
      ...
      if (btf_kind == BTF_KIND_STRUCT || btf_kind == BTF_KIND_UNION)
	{
	  ...
	  for (dmd = type->dtd_u.dtu_members;
	       dmd != NULL; dmd = (ctf_dmdef_t *) ctf_dmd_list_next (dmd))
	    btf_preprocess_sou_member (ctfc, dmd);
	}
    }
}

/* Finish converting a type from CTF encoding to BTF encoding. Return the BTF
   type kind, for convenience.  */

static unsigned int
btf_encode_type (ctf_dtdef_ref dtd)
{
  unsigned int btf_kind, btf_kflag, btf_vlen, btf_size_type;
  unsigned int ctf_info = dtd->dtd_data.ctti_info;
  ...
  dtd->dtd_data.ctti_info = BTF_TYPE_INFO (btf_kind, btf_kflag, btf_vlen);
...
{
  gcc_assert (dtd->dtd_type <= ctfc->ctfc_num_types);

  /* Re-encode the type into BTF.  */
  unsigned int btf_kind = btf_encode_type (dtd);
...
{
  size_t num_ctf_types = ctfc->ctfc_num_types;
  ...
  if (num_ctf_types)
    {
      ...
      for (i = 1; i <= ctfc->ctfc_num_types; i++)
...
{
  unsigned int btf_kind = BTF_INFO_KIND (dtd->dtd_data.ctti_info);
  unsigned int btf_kflag = BTF_INFO_KFLAG (dtd->dtd_data.ctti_info);
  unsigned int btf_vlen = BTF_INFO_VLEN (dtd->dtd_data.ctti_info);

  dw2_asm_output_data (4, dtd->dtd_data.ctti_name, "btt_name");
  dw2_asm_output_data (4, dtd->dtd_data.ctti_info,
  ...
  dw2_asm_output_data (4, dtd->dtd_data.ctti_type, "btt_type");
}
...
{
  dw2_asm_output_data (4, ds.name_offset, "btt_name");
  dw2_asm_output_data (4,
		       BTF_TYPE_INFO (BTF_KIND_DATASEC, 0, ds.entries.length ()),
		       "btt_info");
  /* Note: the "total section size in bytes" is emitted as 0 and patched by
     loaders such as libbpf.  */
  dw2_asm_output_data (4, 0, "btt_size or btt_type");
...
output_btf_header (ctf_container_ref ctfc)
{
   ...
   if (!ctfc_is_empty_container (ctfc))
	 ...
	 {
	   datasec_vlen_bytes
	     += (datasecs[i].entries.length ()) * sizeof (struct btf_var_secinfo);
	 }

       /* Total length (bytes) of the types section.  */
       type_len = (ctfc->ctfc_num_types * sizeof (struct btf_type))
	 + (ctfc->ctfc_num_stypes * sizeof (struct btf_type))
	 + datasec_vlen_bytes
...
output_asm_btf_vlen_bytes (ctf_container_ref ctfc, ctf_dtdef_ref dtd)
{
  ...
  btf_kind = BTF_INFO_KIND (dtd->dtd_data.ctti_info);
...
{
  size_t i;
  size_t num_types = ctfc->ctfc_num_types;
  if (num_types)
    {
...
btf_init_postprocess (void)
{
  ...
  /* Repurpose these counters in the CTF container. For our purposes:
     - ctfc_num_types is the length of ctfc_types_list, after this function.
     - ctfc_num_stypes is the total number of "synthetic" types, types
       generated for BTF output which have no CTF analogue: BTF_KIND_VAR,
       BTF_KIND_FUNC and BTF_KIND_DATASEC types.  */
  tu_ctfc->ctfc_num_types = 0;
  tu_ctfc->ctfc_num_stypes = 0;
  ...
  if (num_ctf_types)
	...
	(tu_ctfc);
      ...
      btf_preprocess_sous (tu_ctfc);

      /* Adjust type IDs and compact the type list so that references-by-ID
	 remain valid.  */
      for (i = 1; i <= num_ctf_types; i++)
	{
	  ctf_dtdef_ref dtd = tu_ctfc->ctfc_types_list [i];
	  btf_adjust_type_ref (dtd);
	  tu_ctfc->ctfc_types_list [i] = NULL;
	  if (dtd->dtd_type < BTF_MAX_TYPE && dtd->dtd_type != BTF_VOID_TYPEID)
	    {
	      tu_ctfc->ctfc_types_list [dtd->dtd_type] = dtd;
	      tu_ctfc->ctfc_num_types ++;
	    }
	}
    }
}
