...
static vec<struct datasec> datasecs;
...
/* Functions actually get two BTF types in the ouput, keep track of them here
   for reference.  */
			...
			struct btf_var_secinfo info)
{
  ...
  uint32_t str_off;
  ctf_add_string (ctfc, secname, &str_off, 0 /*aux string?*/);
  if (strcmp (secname, ""))
...
btf_collect_datasec (ctf_container_ref ctfc)
{
  ...
  FOR_EACH_VARIABLE (node)
	...
	info.type = *var_id + ctfc->ctfc_types->elements () + 1;
      else
	continue;
...
static void
btf_adjust_datasec_refs (struct datasec *ds)
{
  for (size_t j = 0; j < ds->entries.length (); j++)
    {
      struct btf_var_secinfo *info = &(ds->entries[j]);
      info->type = btf_adjust_type_id (info->type);
    }
}

/* After preprocessing, a final pass over all types and variables to adjust
   references-by-ID to account for types which will not be emitted. This is
   essential to ensure references remain correct -- IDs are used to index
   into the type list.  */
...
btf_adjust_refs (ctf_container_ref ctfc)
{
  ...
  size_t num_vars = num_vars_added;
  if (num_types)
    {
  ...
  for (i = 0; i < datasecs.length (); i++)
    btf_adjust_datasec_refs (&datasecs[i]);
}
...
btf_preprocess_sou_member (ctf_container_ref ctfc, ctf_dmdef_t *dmd)
{
  ...
  if (CTF_V2_INFO_KIND (ref_type->dtd_data.ctti_info) == CTF_K_SLICE)
      ...
      if (bits > 0xff || sou_offset + word_offset > 0xffffff)
	{
	  ...
	  ctfc->ctfc_num_vlen_bytes -= sizeof (struct btf_member);
	}
    }

  /* Don't output members which refer to deleted types.  */
...
/* Preprocessing calback routine for variables.  */

int
btf_dvd_preprocess_cb (ctf_dvdef_ref *slot, ctf_container_ref arg_ctfc)
{
...
int
btf_dtd_preprocess_cb (ctf_dtdef_ref *slot, ctf_container_ref arg_ctfc)
{
  ...
  if (btf_kind == BTF_KIND_UNKN)
  ...
  else if (btf_kind == BTF_KIND_FUNC_PROTO)
    {
	 ...
	 These will be output later.  */
      ...
      func_dtd->dtd_data = ctftype->dtd_data;
      func_dtd->dtd_data.ctti_type = ctftype->dtd_type;

      funcs.safe_push (func_dtd);

      /* Only the BTF_KIND_FUNC type actually references the name. The
	 BTF_KIND_FUNC_PROTO is always anonymous.  */
      ctftype->dtd_data.ctti_name = 0;
    }
...
/* Preprocess the CTF information to prepare for BTF output.  BTF is almost a
   ...
   - ctfc->ctfc_types_list holds all types other than BTF_KIND_FUNC and
     BTF_KIND_DATASEC types, including types which will not be output.
     During asm'out, types which map to BTF_KIND_UNKN will be skipped.
...
static void
btf_asm_datasec_type (struct datasec ds)
{
...
output_btf_header (ctf_container_ref ctfc)
{
   ...
   uint32_t datasec_len = 0;

   if (! is_empty_container (ctfc))
     {
       for (size_t i = 0; i < datasecs.length (); i++)
	 {
	   ctfc->ctfc_num_stypes += 1;
	   datasec_len
       ...
       /* CTF types and variables both map to BTF types.
	  Here we also account for:
	  - BTF_KIND_FUNC records
	  - BTF_KIND_DATASEC records
	  both of which are added after num_vlen_bytes is calculated, and so
	  are not included in that value.  */
...
output_asm_btf_func_args_list (ctf_container_ref ctfc,
			       ctf_dtdef_ref dtd)
{
  size_t farg_name_offset = get_cur_ctf_str_len (ctfc);
  ctf_func_arg_t * farg;
  for (farg = dtd->dtd_u.dtu_argv;
       farg != NULL; farg = (ctf_func_arg_t *) ctf_farg_list_next (farg))
