...
report_inline_failed_reason (struct cgraph_edge *e)
{
  if (dump_enabled_p ())
    {
      dump_printf_loc (MSG_MISSED_OPTIMIZATION, gimple_location (e->call_stmt),
		       "  not inlinable: %s -> %s, %s\n",
		       e->caller->dump_name (), 
		       e->callee->dump_name (),
				   ...
				   int depth)
{
  ...
  if (!want_inline && dump_enabled_p ())
    dump_printf_loc (MSG_MISSED_OPTIMIZATION, gimple_location (edge->call_stmt),
		    ...
		    vec<cgraph_edge *> *new_edges)
{
  ...
  if (dump_enabled_p ())
    dump_printf_loc (MSG_NOTE, gimple_location (edge->call_stmt),
		     "\n   Inlined %i times, "
  ...
  while (!edge_heap.empty ())
      ...
      if (dump_enabled_p ())
	{
	  ...
	  dump_printf_loc (MSG_OPTIMIZED_LOCATIONS,
			   gimple_location (edge->call_stmt),
			   " Inlined %s into %s which now has time %f and "
...
early_inline_small_functions (struct cgraph_node *node)
{
  ...
  for (e = node->callees; e; e = e->next_callee)
	...
	continue;
      ...
      if (dump_enabled_p ())
	dump_printf_loc (MSG_NOTE, gimple_location (e->call_stmt),
